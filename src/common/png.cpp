...
namespace v {
    ...
    GxmTexture* load_png_internal (const void *io_ptr, png_rw_ptr read_data_fn, GxmContext* ctx) {
      ...
      GxmTexture* tex = new GxmTexture(width, height, SCE_GXM_TEXTURE_FORMAT_A8B8G8R8, false, ctx);
      if(tex->bad()) {
          delete tex;
          png_destroy_info_struct(png_ptr, &info_ptr);
          free(row_ptrs);
      ...
      printf("Setting texture data\n");
    	png_read_image(png_ptr, row_ptrs);
      printf("Finished setting texture data\n");
    ...
    GxmTexture* load_png (const char* filename, GxmContext* ctx) {
        ...
        return load_png_internal((void *)&fd, read_png_file_fn, ctx);
    }
};
