...
namespace v {
        ...
        m_theme(theme)
    {
        ...
        if (m_shader) {
            ...
            m_vertices = new GxmBuffer(vertCount * sizeof(xmbVertex));
    ...
    xmbVertex* XmbWave::vertexAt(u32 x, u32 z) {
        if(x < 0 || x >= m_width || z < 0 || z >= m_length) return NULL;
        return &((xmbVertex*)m_vertices->data())[(z * m_length) + x];
    }
    ...
    void XmbWave::update (f32 dt) {
        ...
        for(u32 x = 0;x < m_width;x++) {
            for(u32 z = 0;z < m_length;z++) {
                xmbVertex* c = vertexAt(x    , z    );
                xmbVertex* l = vertexAt(x - 1, z    );
                xmbVertex* r = vertexAt(x + 1, z    );
                xmbVertex* t = vertexAt(x    , z - 1);
                xmbVertex* b = vertexAt(x    , z + 1);
